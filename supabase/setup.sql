...
CREATE OR REPLACE FUNCTION decrement_building_checkins(building_id BIGINT) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN UPDATE buildings SET checkin_count = GREATEST(checkin_count - 1, 0) WHERE id = building_id; END;
$$;
...
CREATE POLICY "Users can delete own checkin images" ON storage.objects FOR DELETE USING (bucket_id = 'checkins');
